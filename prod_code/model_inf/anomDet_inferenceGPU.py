...
type_nm = 'combat' # inf or combat
refire = True 

df = pd.read_parquet(f'/homes/dkurtenb/projects/russat/output/MODEL_{type_nm}_tle_data_outlier.parquet')
...
def build_anom_model(NORAD_ID_NUM, type_nm):
    ...
    os.makedirs(f"/homes/dkurtenb/projects/russat/output/inference/{type_nm}_plots/plots_{type_nm}_{NORAD_ID_NUM}", exist_ok=True)
    ...
    detector, anomalies, explanations, timestamps, anomaly_details = run_aeanchor_anomaly_detection(
        orb_df,
        feature_names=feature_names,
        model_path="/homes/dkurtenb/projects/russat/output/model_training/aeanchor/anomaly_model_AEAnchor_4year",
        should_train=False,
...
models = os.listdir("/homes/dkurtenb/projects/russat/output/model_training/aeanchor/anomaly_model_AEAnchor_4year")
...
for count, x in enumerate(id_lst, 1):
    ...
    header = count == 1
    full_df.to_csv(f'/homes/dkurtenb/projects/russat/output/inference/data/anom_df_ae_anchor_{type_nm}_all_sats_0.csv', mode=mode, header=header, index=False)
